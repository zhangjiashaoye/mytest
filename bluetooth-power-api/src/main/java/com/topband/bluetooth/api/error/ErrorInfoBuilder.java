...
public class ErrorInfoBuilder implements HandlerExceptionResolver, Ordered {
     ...
     * 构建错误信息.(ErrorInfo)
     */
    public ErrorInfo getErrorInfo(HttpServletRequest request, Throwable ex) {
        ...
        errorInfo.setStackTrace(getStackTraceInfo(ex, isIncludeStackTrace(request)));
        
        if (ex.toString() != null && ex.toString().indexOf("ExpiredJwtException") > 0) {
			errorInfo.setStatusCode(999);
			errorInfo.setError("timeout");
		} else {
