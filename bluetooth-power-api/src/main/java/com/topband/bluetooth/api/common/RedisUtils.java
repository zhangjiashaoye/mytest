...
public class RedisUtils<T> {
    ...
    /**
     * 对hash类型的数据操作  redisTemplate.opsForHash();
     *
     * 对redis k-V 类型数据操作 redisTemplate.opsForValue();
     *
     * 对链表类型的数据操作  redisTemplate.opsForList();
     *
     * 对无序集合类型的数据操作 redisTemplate.opsForSet()
     *
     * 对有序集合类型的数据操作 redisTemplate.opsForZSet();
     */


    public RedisUtils(RedisTemplate<String, T> _redisTemplate)
    {
        redisTemplate=_redisTemplate;
    }
    RedisTemplate<String, T> redisTemplate;

    public HashOperations<String, String, T> GetHashOp() {
    ...
    public T hGet(String key, String field) {
        return GetHashOp().get(key, field);
    }
     ...
     * @return
     */
    public List<T> hMultiGet(String key, Collection<String> fields) {
        return GetHashOp().multiGet(key, fields);
    }

    public void hPut(String key, String hashKey, T value) {
        GetHashOp().put(key, hashKey, value);
    }

    public void hPutAll(String key, Map<String, T> maps) {
        GetHashOp().putAll(key, maps);
    }
    ...
    public Boolean hPutIfAbsent(String key, String hashKey, T value) {
        return GetHashOp().putIfAbsent(key, hashKey, value);
    }
     ...
     * @return
     */
    public Long hDelete(String key, T... fields) {
        return GetHashOp().delete(key, fields);
    }
    ...
    public boolean hExists(String key, String field) {
        return GetHashOp().hasKey(key, field);
    }
    ...
    public Long hIncrBy(String key, String field, long increment) {
        return GetHashOp().increment(key, field, increment);
    }
    ...
    public Double hIncrByFloat(String key, String field, double delta) {
        return GetHashOp().increment(key, field, delta);
    }
     ...
     * @return
     */
    public Cursor<Map.Entry<String, T>> hScan(String key, ScanOptions options) {
        return GetHashOp().scan(key, options);
    }
